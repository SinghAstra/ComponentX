    ...
    async function initializeConfig() {
        ...
        if (fs.existsSync(configPath)) {
            const response = await prompts({
                type: "confirm",
                name: "overwrite",
                message: pc.yellow(`⚠️  ${CONFIG_FILE_NAME} already exists. Overwrite?`),
                initial: false,
            });
        ...
        setTimeout(() => {
            ...
            spinner.succeed(pc.green(`Initialized ${pc.bold(CONFIG_FILE_NAME)} successfully!`));
...
if (command === "add") {
    ...
    if (!fs.existsSync(configPath)) {
        ...
        console.log(`Run ${pc.cyan("componentx init")} first to configure your project.`);
        process.exit(1);
    }
    ...
    async function fetchRegistryAndInstall() {
        console.log("");
        const fetchSpinner = ora(`Fetching ${pc.cyan(componentName)} from registry...`).start();
        ...
        try {
            ...
            if (!component || !component.files) {
                fetchSpinner.fail(pc.red(`Invalid component data received for '${componentName}'.`));
                process.exitCode = 1;
                return;
            }
            ...
            for (const file of component.files) {
                ...
                writeSpinner.succeed(`Installed ${pc.bold(file.name)} in ${pc.dim(config.componentPath)}`);
            }
            // Handle Dependencies
            if (component.dependencies && component.dependencies.length > 0) {
                console.log("");
                const deps = component.dependencies.join(", ");
                const depSpinner = ora(`Installing dependencies: ${pc.cyan(deps)}...`).start();
                try {
                    ...
                    console.log(pc.dim("Please manually run: ") +
