...
program
        ...
        const { confirmOverwrite } = await inquirer.prompt([
            {
                ...
                message: chalk.yellow(`Configuration file (${CONFIG_FILE_NAME}) already exists. Overwrite it?`),
                default: false,
            },
        ]);
        if (!confirmOverwrite) {
    ...
    try {
        ...
        console.error(chalk.red(`\n❌ Error writing file: ${configPath}`), writeError);
    }
});
...
program
    ...
    .argument("<component-name>", 'The name of the component to download (e.g., "button", "card").')
    ...
    try {
        ...
        console.error(chalk.red(`\n❌ Network or fetch error: ${fetchError instanceof Error
            ? fetchError.message
            : String(fetchError)}`));
        return;
    }
    ...
    try {
        ...
        console.error(chalk.red(`\n❌ Failed to create directory ${fullDestDir}: ${dirError instanceof Error ? dirError.message : String(dirError)}`));
        return;
    }
    ...
    try {
        ...
        console.error(chalk.red(`\n❌ Error writing file: ${destFilePath}`), writeError);
    }
});
